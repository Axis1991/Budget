...
class Expense:
    ...
    def __eq__(self, other):
        return self.id == other.id and self.amount == other.amount and self.description == other.description
...
def read_expenses(filename: str, expense_list) -> list[Expense]:
    with open(filename, encoding="utf-8") as stream:
        ...
        try:
            ...
            expenses_csv = [Expense(find_next_id(expense_list), float(expense.amount), expense.description) for expense in expenses_no_id]
            return expenses_csv
...
def print_expenses(expense_list: list[Expense]) -> None:
    ...
    for item in expense_list:
        ...
        if item.amount >= 1000:  #float?
            ...
            big = " "
        print(f"{item.id:^6}    {strip_zeros(item.amount):<8}   {big:^6}  {item.description:<20}")
